...
const STEPS = ["Objekt", "Ertraege", "Parkplaetze", "Lage", "Ueberpruefen"];
...
export default function FormWizard({ onComplete, saving }: FormWizardProps) {
  ...
  const liveResult = (() => {
    ...
    try {
      return calculateValuation({
        city: property.city, condition: property.condition,
  ...
  function validate(): boolean {
    ...
    if (step === 1) { if (!valuation.rent_residential) e.rent_residential = "Pflichtfeld"; }
    setErrors(e);
    ...
    <div className="max-w-xl mx-auto">
        ...
        {step === 0 && (
          <div className="space-y-4">
            ...
            {property.city && (() => {
              ...
              return m ? <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-xs text-blue-700"><strong>Auto-Lageklasse:</strong> {m.type} - {m.name}, {m.canton} ({m.population.toLocaleString("de-CH")} Einw.)</div> : null;
            })()}
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {liveResult && (liveResult.sollIstDiffWohnen > 0 || liveResult.sollIstDiffGewerbe > 0) && (
              <div className="bg-amber-50 border border-amber-300 rounded-lg px-4 py-3 text-xs text-amber-700">
                <strong>Mietpotenzial:</strong> Wohnen +{formatCHF(liveResult.sollIstDiffWohnen)}/Jahr{liveResult.sollIstDiffGewerbe > 0 && `, Gewerbe +${formatCHF(liveResult.sollIstDiffGewerbe)}/Jahr`} bei Neuvermietung.
              </div>
            )}
            <div className="grid grid-cols-2 gap-3">
              <FField label="Leerstandsquote aktuell (%)" value={valuation.vacancy_rate} onChange={v => updV("vacancy_rate", v)} placeholder="0" />
              <FField label="Leerstand letzte 5 Jahre (%)" value={valuation.vacancy_avg5y} onChange={v => updV("vacancy_avg5y", v)} placeholder="0" note="Fuer nachhaltige Ertragsbasis" />
            </div>
            <div className="grid grid-cols-2 gap-3">
            ...
            {liveResult && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[["Brutto-Soll", formatCHF(+valuation.rent_residential + +valuation.rent_commercial)], ["Eff. Ertrag", formatCHF(liveResult.effectiveIncome)], ["Kap.-Satz", formatPct(liveResult.capRateBreakdown.final)]].map(([l, v]) => (
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FField label="Abstellplaetze Aussen (AAP)" value={valuation.aap_count} onChange={v => updV("aap_count", v)} placeholder="0" note="Offene oder ueberdachte Aussenparkplaetze" />
            ...
            {liveResult && (+valuation.aap_count > 0 || +valuation.ehp_count > 0) ? (
              <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><p className="text-xs text-gray-500">Lagekategorie</p><p className="font-semibold text-gray-800">{liveResult.locationCategory === "sehrStark" ? "Staedtisch" : liveResult.locationCategory === "gut" ? "Agglomeration" : "Laendlich"}</p></div>
                  <div><p className="text-xs text-gray-500">Parkplatz-Ertrag p.a.</p><p className="font-bold text-green-700">{formatCHF(liveResult.parkingIncome)}</p></div>
                </div>
              </div>
            ) : (
              <div className="bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-xs text-gray-500">Keine Parkplaetze - Ertrag wird nicht eingerechnet.</div>
            )}
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={valuation.notes} onChange={e => updV("notes", e.target.value)} className="input-field resize-none" rows={3} placeholder="Optionale Anmerkungen..." />
            </div>
            {property.city && (() => {
              const m = MUNICIPALITIES.find(x => x.name.toLowerCase() === property.city.toLowerCase());
              return (
          ...
          <div className="space-y-4">
            ...
            {liveResult && (
              <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl p-5 text-white">
                ...
                {liveResult.parkingIncome > 0 && <p className="text-blue-200 text-xs mt-1">inkl. Parkplatz-Ertrag: {formatCHF(liveResult.parkingIncome)}/Jahr</p>}
              </div>
            )}
          </div>
        )}
      ...
      <div className="flex justify-between items-center mt-5">
        ...
        {step < STEPS.length - 1
          ...
          : <button onClick={handleSubmit} disabled={saving || !liveResult} className="btn-accent px-6 py-2.5">{saving ? "Wird gespeichert..." : "Bewertung speichern"}</button>
        }
      </div>
    </div>
  );
