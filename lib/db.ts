...
export async function upsertProfile(userId: string, updates: { full_name?: string; company?: string; phone?: string }): Promise<Profile | null> {
...
export async function updateProperty(id: string, updates: Partial<Omit<Property, "id" | "user_id" | "created_at">>): Promise<Property | null> {
...
export async function updateValuation(id: string, updates: Partial<Omit<Valuation, "id" | "user_id" | "created_at">>): Promise<Valuation | null> {
  const { data, error } = await supabase.from("valuations").update(updates).eq("id", id).select().single();
